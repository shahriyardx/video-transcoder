...
export const consumeMessages = async (queue: string, exchange: string, routing_key: string) => {
    const connection = await amqp.connect("amqp://localhost:5672")
    const channel = await connection.createChannel();
    await channel.assertQueue(queue, { durable: false })

    console.log(`[*] Waiting for messages (Exchange: ${exchange}, Routing Key: ${routing_key})`);
...
// Usage
consumeMessages("upload_queue", "upload_exchange", "file.uploaded").catch(console.error);
